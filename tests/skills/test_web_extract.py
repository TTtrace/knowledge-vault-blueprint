...
class WebExtractTests(unittest.TestCase):
    ...
    def test_static_wechat_quality_rejection_then_browser_manual_orders_diagnostics(self):
        # Static WeChat quality rejection must be retained before the browser
        # manual challenge in ordered, sanitized diagnostics.
        with mock.patch.object(
            web_extract, "_validate_url",
            side_effect=lambda url, policy=None: url,
        ), mock.patch.object(
            web_extract, "static_fetch",
            side_effect=lambda url, policy=None: ("https://mp.weixin.qq.com/s/x", "text/html", "<html></html>"),
        ), mock.patch.object(
            web_extract, "extract_wechat",
            side_effect=lambda html, url: web_extract.ExtractionResult(
                title="标题", markdown="", method="wechat-static"
            ),
        ), mock.patch.object(
            ...
            with self.assertRaises(web_extract.ExtractionError) as ctx:
                web_extract.extract_article("https://mp.weixin.qq.com/s/x", policy=PERMISSIVE)
            self.assertEqual(ctx.exception.state, "manual")
            self.assertEqual(ctx.exception.methods_attempted, ["static-fetch", "browser"])
            diags = ctx.exception.attempt_diagnostics
            self.assertEqual(diags[0]["method"], "static-fetch")
            self.assertEqual(diags[0]["outcome"], "ok")
            self.assertEqual(diags[1]["method"], "wechat-static")
            self.assertEqual(diags[1]["outcome"], "rejected")
            self.assertEqual(diags[1]["reason_code"], "body_empty")
            self.assertEqual(diags[2]["method"], "browser")
            self.assertEqual(diags[2]["outcome"], "manual")
            self.assertEqual(diags[2]["reason_code"], "verification_required")
            self.assertEqual(diags[2]["message"], "需要验证")

    def test_unknown_exception_text_never_persists_sentinel(self):
        # An unknown/raw exception reason must collapse to a generic fixed
        # reason; the secret must never appear in persisted diagnostics.
        sentinel = "SENTINEL-SECRET-9382"
        with mock.patch.object(
            web_extract, "_validate_url",
            side_effect=lambda url, policy=None: url,
        ), mock.patch.object(
            ...
            web_extract, "_try_playwright",
            side_effect=web_extract.ExtractionError(f"unexpected {sentinel} traceback", state="failed"),
        ):
            with self.assertRaises(web_extract.ExtractionError) as ctx:
                web_extract.extract_article("https://mp.weixin.qq.com/s/x", policy=PERMISSIVE)
            blob = json.dumps(ctx.exception.attempt_diagnostics, ensure_ascii=False)
            self.assertNotIn(sentinel, blob)
            self.assertNotIn("traceback", blob)
            browser_diag = ctx.exception.attempt_diagnostics[-1]
            self.assertEqual(browser_diag["reason_code"], "unknown_error")
            self.assertEqual(browser_diag["message"], "未知错误")

    def test_no_profile_uses_ephemeral_context(self):
        # Without a configured profile the fallback must use an ephemeral
        # launch + new_context, never launch_persistent_context.
        calls = {"ephemeral": False, "persistent": False}
        ...
        with mock.patch.object(
            web_extract, "_validate_url", side_effect=lambda url, policy=None: url
        ), mock.patch.object(
            web_extract, "_make_navigation_guard", lambda policy=None: lambda route: None
        ), mock.patch(
            "playwright.sync_api.sync_playwright", return_value=FakePlaywright()
        ):
            web_extract.playwright_fetch("https://example.com/", policy=PERMISSIVE)
        self.assertTrue(calls["ephemeral"])
        self.assertFalse(calls["persistent"])

    def test_profile_dir_uses_persistent_context_path_not_in_diagnostics(self):
        # With a configured profile, launch_persistent_context must receive the
        # exact profile dir, and the path must never appear in diagnostics.
        profile = "/tmp/some/private/profile-dir"
        seen = {"persistent": None}
        ...
        class FakeChromium:
            def launch(self, **k):
                raise AssertionError("ephemeral launch must not be used with a profile")

            def launch_persistent_context(self, profile_dir, **k):
                seen["persistent"] = profile_dir
                return FakeContext(FakePage())
        ...
        with mock.patch.object(
            ...
            web_extract.playwright_fetch("https://example.com/", profile_dir=profile, policy=PERMISSIVE)
        self.assertEqual(seen["persistent"], profile)
