...
- 所有对外网络目标统一走共享策略 `network_security.py`。URL 必须是绝对 HTTP(S) 且使用 DNS 主机名；任何 IPv4/IPv6 字面量（含公网与 Fake-IP 字面量）在 `stage` 与每个网络边界被拒绝。
...
- 默认模式要求系统解析结果全部全局可路由。仅当 `VAULT_CAPTURE_SSRF_FAKE_IP_MODE=clash` 且 `VAULT_CAPTURE_SSRF_DOH_PROVIDER=cloudflare|google` 同时设置时，系统返回 `198.18.0.0/15` 仅作为代理信号，通过固定信任的 DoH 独立查询 A/AAAA，至少一个地址且全部全局才放行；Fake-IP 绝不作为真实目标。配置缺失/部分/未知、DoH 超时/TLS/HTTP/DNS 非成功/畸形/超限/无地址/任一非全局均失败关闭，不退回私有访问，且拒绝直接以 Fake-IP/公网 IP 字面量作为 URL。
...
6. 网页抓取安全默认严格、无需任何环境变量。仅在 Clash/FlClash TUN Fake-IP 代理环境下，为通过 `198.18.0.0/15` 的系统解析，可在测试 agent/Gateway 显式注入 `VAULT_CAPTURE_SSRF_FAKE_IP_MODE=clash` 与 `VAULT_CAPTURE_SSRF_DOH_PROVIDER=cloudflare|google`（两值固定，同时设置才生效）。生产与测试配置隔离：测试仅面向 basename 以 `-test` 结尾的 Vault 与独立 agent/Gateway，验证后恢复既有值并重载测试 Gateway；不把 Fake-IP 感知配置写入生产运行配置。DoH provider 端点固定于代码，不接受任意 URL。